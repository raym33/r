...
class AndroidSkill(Skill):
    ...
    def android_sms_send(self, phone: str, message: str) -> str:
        ...
        if self._adb:
            ...
            success, output = self._run_adb(
                ...
                "-d", f"sms:{phone}", "--es", "sms_body", f'"{message}"'
            )
            return json.dumps({
                "success": success,
                "phone": phone,
    ...
    def android_sms_list(
        ...
        if self._adb:
            success, output = self._run_adb(
                ...
                "--projection", "address:body:date", "--limit", str(limit)
            )
            if success:
                messages = []
    ...
    def android_photo(
        ...
        if self._adb:
            ...
            success, out = self._run_adb(
                ...
                "--ez", "android.intent.extra.USE_FRONT_CAMERA", "true" if camera == "front" else "false"
            )
            return json.dumps({
                "success": success,
    ...
    def android_notification(
        ...
        return self._bridge_result("notification", {
            "title": title,
            "message": message,
            "priority": priority,
        })
    ...
    def android_volume(
        ...
        if self._adb:
            ...
            if level is not None:
                ...
                success, output = self._run_adb(
                    "shell", "media", "volume", "--stream", stream_id,
                    "--set", str(adb_level)
                )
                return json.dumps({
                    "success": success,

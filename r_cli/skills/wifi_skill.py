...
class WiFiSkill(Skill):
    ...
    def _get_interface(self) -> Optional[str]:
        ...
        if self._system == "linux":
            ...
            # Try nmcli
            if self._nmcli:
                success, output = self._run_command([self._nmcli, "-t", "-f", "DEVICE,TYPE", "device"])
                if success:
                    for line in output.split("\n"):
                        if ":wifi" in line:
                            return line.split(":")[0]
            ...
            if success:
                ...
                for line in lines:
                    ...
                    if len(parts) >= 2:
                        ...
                        ssid = parts[0]
                        signal = int(parts[1]) if len(parts) > 1 and parts[1].lstrip("-").isdigit() else 0
                        networks.append({
                            "ssid": ssid,
                            "signal": signal,
                            "security": "Unknown",
    ...
    def wifi_disconnect(self, interface: Optional[str] = None) -> str:
        ...
        if self._system == "linux" and self._nmcli:
            ...
            self._run_command([
                self._networksetup, "-setairportpower", interface or "en0", "on"
            ])
        else:
            ...
            if success:
                for line in output.split("\n"):
                    if line.startswith("yes:"):
                        ...
                        status["signal"] = int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else None
    ...
    def wifi_hotspot(
        ...
        if action == "start":
            ...
            # Create hotspot
            success, output = self._run_command([
                self._nmcli, "device", "wifi", "hotspot",
                "ssid", ssid,
                "password", password,
            ])
            ...
            success, output = self._run_command([
                self._nmcli, "connection", "down", "Hotspot"
            ])
